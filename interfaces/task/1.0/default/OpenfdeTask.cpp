...
Return<void> OpenfdeTask::setFocusedTask(uint32_t taskID) {
...
Return<void> OpenfdeTask::removeTask(uint32_t taskID) {
    bool ret;
    if (mActivityTaskManager == nullptr) {
        sp<IBinder> binderTask = android::defaultServiceManager()->getService(android::String16("activity_task"));
        if (binderTask != nullptr)
...
Return<void> OpenfdeTask::removeAllVisibleRecentTasks() {
    if (mActivityTaskManager == nullptr) {
        sp<IBinder> binderTask = android::defaultServiceManager()->getService(android::String16("activity_task"));
        if (binderTask != nullptr)
            mActivityTaskManager = android::interface_cast<IActivityTaskManager>(binderTask);
...
Return<void> OpenfdeTask::getAppName(const hidl_string& packageName, getAppName_cb _hidl_cb) {
    ...
    const char* OutAppName = android::String8(AppName).c_str();
...
Return<void> OpenfdeTask::commitText(const hidl_string& text) {
...
Return<void> OpenfdeTask::sendKeyEvent(int32_t action, int32_t code) {
